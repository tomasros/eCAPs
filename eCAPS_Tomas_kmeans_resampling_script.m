...
%% EEGdata_one= voxels x timepoints

function silhouette_value_model_one=

M=20; %number of boostraps
percent=0.05; %%percentage of resampled points
reps=5;

for k_number=4:6 %% number of clusters
...
%%kmeans clustering
    disp('kmeans on GPU');
    tic %%Tomas edit
    [idx,k_model_one] = kmeans(gpuArray(EEGdata_one'),k_number,'distance','correlation','replicates',reps,'empty','drop','maxiter',500);
    idx=gather(idx);
