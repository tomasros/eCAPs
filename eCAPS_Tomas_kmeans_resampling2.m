...
function silhouette_value_model_one=eCAPS_Tomas_kmeans_resampling2(data, k_min, k_max, nbootstraps, data_ratio, kmeansreps)
...
%   
% similarity_broadband=pdist2_adjacency_ecaps(concatenated_k_model_one, concatenated_k_model_one);
% size(similarity_broadband)
% d = eye(size(similarity_broadband), 'logical');similarity_broadband(d)=1; %%ensure diagonal is 1's
% distance_matrix_model_one=1-similarity_broadband;
% distance_matrix_model_one_vectorized = squareform(distance_matrix_model_one,'tovector');
% 
% 
% 
% k_number
% [kaverages_clustering_index_model_two] = kaverages(similarity_broadband,k_number);
% kaverages_eva_two = evalclusters(concatenated_k_model_one,kaverages_clustering_index_model_two','silhouette','ClusterPriors', 'equal','Distance', distance_matrix_model_one_vectorized);
% kaverages_silhouette_value_model_two(:,k_number)=kaverages_eva_two.CriterionValues;
% 
%   
%   bpcombined = [kaverages_silhouette_value_model_two(:)];
% figure; hb = bar(1:1:length(kaverages_silhouette_value_model_two), bpcombined, 'grouped');title('Silhouette values');
% legend('kaverages');
