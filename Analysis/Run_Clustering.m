...
%     [idx,CP] = kmeans(XONn_filtered',n_clusters,'distance','correlation','replicates',n_rep,'empty','drop','maxiter',1000, 'Options',options);
% 
% for time=1:length(idx)
% % time_course(:,time)=nnls(CP', XONn_filtered(:,time));
% time_course(:,time)=tntnn(CP', XONn_filtered(:,time));
% end
% toc
% 
% max(CP')
...
%     XONn_filtered=gpuArray(XONn_filtered);
%      class(XONn_filtered)
%     [idx,CP] = kmeans(XONn_filtered',n_clusters,'distance','correlation','replicates',n_rep,'empty','drop','maxiter',500, 'Options',options);
%     idx=gather(idx);
%     CP=gather(CP);
%     XONn_filtered=gather(XONn_filtered);
%     toc
%     
...
% [time_course, CP]=nnmf(XONn_filtered',n_clusters, 'replicates',6,'algorithm','als','options', opt);
% % [CP,time_course]=CauchyNMF(XONn_filtered,n_clusters,'SCALE',-2,'VERBOSE',1);time_course=time_course';CP=CP';
% % max(time_course)
% % min(time_course)
...
% max(XONn_filtered(:))
% min(XONn_filtered(:))
[idx,Xout,Aout,Yout,numIter,tElapsed,finalResidual]=NMFCluster(XONn_filtered,n_clusters, option);
% [idx,Xout,Aout,Yout,numIter,tElapsed,finalResidual]=NMFCluster(XONn_filtered,n_clusters);

% size(Xout)  %% reconstructed XONn_filtered
...
% idx = smoothing(XONn_filtered,CP',n_clusters,opts);
% idx=idx';


%% option3: reject small segments via Poulsen microstate toolbox
% opts.minTime = 1; %%minimum segment size in samples
% idx = MicroSmooth(XONn_filtered, CP', 'reject segments' ,opts); %% 'reject segments' (default) or 'windowed'.
% idx=idx';

%% option4: windowed smoothing via Poulsen microstate toolbox
% opts.b=1; %%smoothing window in samples (@40Hz 1 sample=25ms)
% opts.lambda=5; %smoothing weight
% idx = MicroSmooth(XONn_filtered, CP', 'windowed' ,opts); %% 'reject segments' (default) or 'windowed'.
% idx=idx';
...
for comp=1:n_clusters
    data_variance = sum(var(XONn_filtered));
