...
function [Consensus_ordered] = CAP_ConsensusClustering_Tomas(X,K_range,Subsample_type,Subsample_fraction,n_folds,DistType)



%%Tomas silhouette score resampling (alternative to consensus clustering
    silhouette_value_model_one=eCAPS_Tomas_kmeans_resampling2(cell2mat(X), min(K_range), max(K_range), n_folds, Subsample_fraction, 20);
...
stop
    ...
    for k = 1:length(K_range)
        ...
        for h = 1:n_folds
            ...
            %  disp(['consensus kmeans on CPU );
%            replicate=h
%            IDX = kmeans(X_ss,K_range(k),'Distance',DistType,'Replicates',10,'Start','uniform');
     
%     disp('consensus kmeans on GPU' );
%     replicate=h
    IDX = kmeans(gpuArray(X_ss),K_range(k),'distance','correlation','replicates',10,'maxiter',500);
    IDX=gather(IDX);
