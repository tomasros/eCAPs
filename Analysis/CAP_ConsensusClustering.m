...
%            fold=h
%            IDX = kmeans(X_ss,K_range(k),'Distance',DistType,'Replicates',10,'Start','uniform', 'Options',options);
     
    disp('consensus kmeans on GPU' );
    fold=h
    IDX = kmeans(gpuArray(X_ss),K_range(k),'distance','correlation','replicates',10,'Start','uniform','empty','drop','maxiter',500);
    IDX=gather(IDX);
