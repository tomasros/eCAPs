function [CP3,Disp,Std_Clusters,idx,d,sfrac] = Run_Clustering_Tomas_ICASSO(XONn,n_clusters,mask,brain_info,maskP,maskN,n_rep,idx_sep_seeds,SeedType)
...
%%%%NEW OPTION Tomas: resampled ICASSO kmeans (from multiple runs of smaller subsamples of data which is faster, e.g. 10%, 20%)
%% data=data = voxels x timepoints
data_ratio=maskP; %%in percent
nbootstraps=maskN;%% number of resamples
disp('ICASSO resampled kmeans');
[meanIq,CP] =eCAPS_Tomas_kmeans_ICASSO(XONn, n_clusters, n_clusters, nbootstraps, data_ratio/100, 20);
mean_cluster_quality=meanIq(n_clusters)
spatial_correlation = corr(CP',XONn);
% size(spatial_correlation)
[M,idx]=max(spatial_correlation);
idx=idx';

%%%time-courses for non-negative data (i.e. for non zscored data)
% for time=1:length(idx)
% % time_course(:,time)=nnls(CP', XONn(:,time));
% time_course(:,time)=tntnn(CP', XONn(:,time));
% end

%%%ordinary least squares time-courses for negative data (i.e. for zscored data)
time_course=(XONn'*pinv(CP))';
...
%Tomas z-normalize maps (so they are all on same scale)

CP3=zscore(CP2,0,2); %%zscore across voxels 

% %% NORMALIZE TO percent change
% CP3=CP2./mean(CP2) -1;  %% Tomas column-wise percent change centred around 1
% CP3(isnan(CP3))=0; %Tomas remove NaNs if 0/0


% %% OR NORMALIZE TO range

% CP3 = normalize(CP2,2,'range',  [-2, 2] ); %% keeps data positive between [0.5 1.5]
