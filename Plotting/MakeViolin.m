...
function [box,h_viol,ah] = MakeViolin(Y,ah,Lab,YLabel,Color,n_pop,n_states)
    ...
    h_viol = distributionPlot(ah,Y','showMM',0,'color',Col_final,'yLabel',YLabel,'xNames',Lab_final, 'addSpread', 0);
    h_viol=0;
...
%     size(Y', 1)
    ...
    hold(ah,'on');
    if size(Y', 1)==1 
    box = boxplot([],'Parent',ah,'Labels',Lab_final, 'symbol', '');    
    else
    box = boxplot(Y','Parent',ah,'Labels',Lab_final, 'symbol', '');
    end
    
    h_box = findobj(box,'Tag','Box');
    set(h_box,'color','k','LineWidth',1);
%     
...
%             set(h_outliers(idx_oi),'Marker','.','MarkerFaceColor',Col_final2{n_pop*n_states-idx_oi+1},...
%                 'MarkerEdgeColor',Col_final2{idx_oi},'MarkerSize',1);
% 
%         end
%     end
