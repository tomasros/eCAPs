...
%
% Version 2.0 eCAPS (March 2021): adaptation for 4D source-space EEG
% data (e.g. from Brainstorm SPM export) and ICASSO resampling (Tomas Ros,
...
toolbox_file_location=which('eCAPS_Tomas_kmeans_ICASSO.m');
toolbox_path=toolbox_file_location(1:end-28);
...
handles.PopColor{1} = [0, 0, 0; 0.3010 0.7450 0.9330;0.9290 0.6940 0.1250; 0 1 0];
handles.PopColor{2} = [0, 0, 0; 0 0.4470 0.7410; 0.8500 0.3250 0.0980; 0.4660 0.6740 0.1880] ;
...
if strcmp(handles.prefix, 'sw') == 1
        FFiles = cellstr(spm_select('List',fullfile(tmp),['^' '.*\.' 'nii' '$']));
else
    ...
    try
        ...
        switch get(handles.MaskPopup,'Value')
            ...
            %GM
            case 1
               a = spm_vol(fullfile('.','DefaultData','default_grey_mask_without_cerebellum.nii'));
...
%     Data_OI = spm_select(Inf,'dir','Select the directories containing the functional data to analyse within the assessed group...');
    ...
    try
        ...
        for i = 1:size(Data_OI,1)
            [~,name,~] = fileparts(Data_OI{i}); 
            disp(['Currently loading subject ',num2str(i),'...', name]);
           
            %%Tomas save file details
            handles.filenames{handles.n_datasets+1}{i,1}=Data_OI{i}; %% extract all filenames for saving
            underlineLocations = strfind(name, 'S');underlineLocations2 = strfind(name, '_');
            subjectID = name(underlineLocations(1)+1:underlineLocations2(1)-1); %extract subject 
            handles.subjectID{handles.n_datasets+1}{i,1}=subjectID;
...
%%%% Tomas read 4D nifti data 
%%read in data
tmp_data=single(spm_read_vols(spm_vol(Data_OI{i})));% %%4D matrix: voxels x voxels x voxels x timepoints 


%%comment for all timepoints 
% minimum_number_of_timepoints = 1200 %%select minimum number of timepoints or comment for all e.g. 4800 = 120 seconds x @40Hz
% tmp_data=tmp_data(:,:,:,1:minimum_number_of_timepoints );





% min_tmp_data=min(tmp_data(:))
% max_tmp_data=max(tmp_data(:)) 

%%reshape
number_of_timepoints=length(tmp_data);
tmp_data=permute(tmp_data,[4 1 2 3]);
tmp_data=reshape(tmp_data,number_of_timepoints, []);  %% tmp_data == timepoints x voxels


%%TOMAS OPTION: select limited number of GFP peaks for Reference population***
 if handles.n_datasets == 0
number_of_peaks=200;
GFP = std(tmp_data,[],2);
[~, GFPpeakidx] = findpeaks(GFP, 'NPeaks', number_of_peaks);
% max(GFPpeakidx)
tmp_data = tmp_data(GFPpeakidx,:); 
number_of_timepoints=size(tmp_data, 1);
disp(['Selecting ' num2str(number_of_timepoints) ' GFP peaks' ]);
 else
 end


%%apply grey-matter mask
tmp_data=tmp_data(:,handles.mask{1});

%%%%
           
%             parfor t = 1:length(FFiles)
...
%%%low-pass filtering (Tomas optional)
% fpass=20 ; %% upper cutoff in Hz
% fs=40; %%sampling rate in Hz
% tmp_data=lowpass(tmp_data,fpass,fs);

%%% Hilbert transform for emplitude envelope (Tomas optional)
% tmp_data=abs(hilbert(tmp_data));
...
% tmp_data=tmp_data./mean(tmp_data);  %% ARE YOU SURE?? Tomas column-wise percent change centred around 1
% tmp_data(isnan(tmp_data))=0; %Tomas remove NaNs if 0/0
...
%             tmp_data = zscore(tmp_data,0,2); %%Tomas temporal zscore 
%             tmp_data = normalize(tmp_data,2,'range',  [1, 3] ); %% keeps data positive between [0 1]
%              tmp_data = normalize(tmp_data,2,'scale'); %% normalise by standard deviation
...
%                 handles.FD
%             catch
                handles.FD{handles.n_datasets+1}(:,i) = zeros(number_of_timepoints,1);
...
function TR_Entry_Callback(hObject, eventdata, handles)
    ...
    if (~isempty(str2double(get(hObject,'String')))) && ...
        ...
        handles.TR = str2double(get(hObject,'String'));  
           handles.TR=1/handles.TR; %%Tomas convert from Hz to TR
        set(hObject,'BackgroundColor', [101,140,196]/255);
        handles.isTROK = true;
...
OverallInfo.SaveDir=handles.savedir;
...
OverallInfo.SubjectFiles=handles.filenames;
OverallInfo.SubjectID=handles.subjectID;
...
Outputs.KMeansClustering.ClusterQuality=handles.cluster_quality; %%Tomas addition
...
Outputs.Metrics.CorrDist=handles.CorrDist;
...
%%Tomas output variables to workspace + save table
clear CAPs
clear CAPs_BIOMARKER_table
assignin('base','OverallInfo', OverallInfo);
assignin('base','Parameters', Parameters);
assignin('base','Outputs', Outputs);
run('Tomas_extract_CAPs_measures_to_table.m');
writetable(BIOMARKER_table,[handles.savedir '\' fancy_name '.csv']); 
...
% Slice plotting itself; 2.0 is the color past which the display saturates
handles.SeedGraphX = plot_slice(handles.seed_display,get(handles.TVIS_Slider,...
    'Value'),1,handles.mask{handles.ReferencePopulation},handles.brain,...
...
% Modifies the text
set(handles.TSeed,'String',['Zscore: ',sprintf('%.1f',get(hObject,'Value'))]);
...
%%start TOMAS option1: perform consensus clustering WITHIN each subject and then average ACROSS subjects (since consensus matrix grows rapidly as nsubjects^2)
% nsubjects= size(handles.Xonp{handles.ReferencePopulation}, 2);
% for n=1:nsubjects
% subject=n    
% [~,Lorena(:,:,n)] = ComputeClusteringQuality(handles.Consensus,2:handles.Kmax);  %% Tomas consensus clustering  
% end
% Lorena=mean(Lorena, 3); %% average across subjects
% Lorena=mean(Lorena, 2); %% average across consensus thresholds
% handles.ConsensusQuality = 1-Lorena;
%%end TOMAS option1:


%%start TOMAS option2: ICASSO resampling (alternative to consensus clustering); see: https://research.ics.aalto.fi/ica/icasso/
cluster_quality = CAP_ConsensusClustering_ICASSO(cell2mat(handles.Xonp{handles.ReferencePopulation}),2:handles.Kmax,'items',handles.PCC/100,handles.n_rep,'correlation');
handles.cluster_quality=squeeze(cluster_quality);
handles.ConsensusQuality=cluster_quality;
cluster_quality
%%end TOMAS option2:
...
%     tmp_plot = bar(2:handles.Kmax,1-Lorena,'Parent',handles.CCPlot); %original consensus clustering
      tmp_plot = bar(1:handles.Kmax,cluster_quality,'Parent',handles.CCPlot);
    ...
    custom_cm = cbrewer('seq','Reds',50);
        custom_cm(custom_cm < 0) = 0;
    colormap(handles.CCPlot,custom_cm(6:50,:));
%        colormap(handles.CCPlot,'jet');
...
%     handles.Log = CAP_AddToLog(handles.Log,'Valid percentage positive voxels chosen',{handles.Pp},{'Percentage positive voxels'}); 
   handles.Log = CAP_AddToLog(handles.Log,'Valid percentage of data chosen',{handles.Pp},{'Percentage of data'}); 
...
%     handles.Log = CAP_AddToLog(handles.Log,'Valid percentage negative voxels chosen',{handles.Pn},{'Percentage negative voxels'});
   handles.Log = CAP_AddToLog(handles.Log,'Valid number of resamples chosen',{handles.Pn},{'Number of resamples'});
...
if handles.is_seed_free == 0
    [handles.CAP,handles.Disp,handles.STDCAP,handles.idx{handles.ReferencePopulation},...
        handles.CorrDist,handles.sfrac] = Run_Clustering_Tomas_ICASSO(cell2mat(handles.Xonp{handles.ReferencePopulation}),...
...
%     [handles.CAP,handles.Disp,handles.STDCAP,handles.idx{handles.ReferencePopulation},...
%         handles.CorrDist,handles.sfrac] = Run_Clustering_Tomas(cell2mat(handles.Xonp{handles.ReferencePopulation}),...
%         handles.K,handles.mask{handles.ReferencePopulation},handles.brain_info{handles.ReferencePopulation},...
%         handles.Pp,handles.Pn,handles.n_rep,1,handles.SeedType);
    
    
    %% Tomas ICASSO resampling approach
[handles.CAP,handles.Disp,handles.STDCAP,handles.idx{handles.ReferencePopulation},...
        handles.CorrDist,handles.sfrac] = Run_Clustering_Tomas_ICASSO(cell2mat(handles.Xonp{handles.ReferencePopulation}),...
        handles.K,handles.mask{handles.ReferencePopulation},handles.brain_info{handles.ReferencePopulation},...
        handles.Pp,handles.Pn,handles.n_rep,1,handles.SeedType);
...
%     handles.CAP=CAP_Zscore(handles.CAP); %%Isik spatial z-score
%       handles.CAP=zscore(handles.CAP); %%Tomas spatial z-score
    

    %Tomas normalize between minimum and maximum
%     minimum = -1.5;
%     maximum =  1.5;
%     
% for i=1:size(handles.CAP,1)
% % handles.CAP(i, :) = (handles.CAP(i,:)-min(handles.CAP(i,:)))/(max(handles.CAP(i,:))-min(handles.CAP(i,:)));
% 
% value = handles.CAP(i, :);
% handles.CAP(i, :) = (maximum-minimum)/(max(value)-min(value))*(value-max(value))+maximum;
% end
%  
...
tmp_cb2 = cbrewer('div','RdBu',100);
tmp_cb2(tmp_cb2 < 0) = 0;
colormap(handles.CAP_Mat,flipud(tmp_cb2));
% colormap(handles.CAP_Mat,'jet');
...
set(handles.TVIS,'String',['Zscore: ',sprintf('%.1f',get(handles.TVIS_Slider,'Value'))]);
...
%     set(tmpF{i_CAP},'String',{['STATE ' num2str(i_CAP)], [num2str(sum(handles.idx{handles.ReferencePopulation}==i_CAP)),'/',...
%         num2str(size(handles.idx{handles.ReferencePopulation},1))],[sprintf('%.1f',...
%         sum(handles.idx{handles.ReferencePopulation}==i_CAP)/size(handles.idx{handles.ReferencePopulation},1)*100),' [%]']});
%     
%%Tomas edit
    set(tmpF{i_CAP},'String',{['STATE ' num2str(i_CAP)],[sprintf('%.1f',...
        sum(handles.idx{handles.ReferencePopulation}==i_CAP)/size(handles.idx{handles.ReferencePopulation},1)*100),' %']});
...
handles.Log = CAP_AddToLog(handles.Log,'Clustering performed',...
    {handles.K,handles.Pp,...
    handles.Pn},{
    'Number of clusters','Percentage of data','Number of resamples'});
...
%             handles.idx{n_ds} = CAP_AssignFrames(handles.CAP,cell2mat(handles.Xonp{n_ds}),handles.CorrDist,handles.percentile)';
            handles.idx{n_ds} = CAP_AssignFrames_Tomas_smoothed(handles.CAP,cell2mat(handles.Xonp{n_ds}),handles.CorrDist,handles.percentile)';
%             handles.idx{n_ds} = CAP_NMFconnectivity(handles.CAP,cell2mat(handles.Xonp{n_ds}),handles.FrameIndices{n_ds}.kept.active);
%         catch
...
handles.Log = CAP_AddToLog(handles.Log,'Timepoint assignment performed',...
    {num2str(handles.percentile)},{'Percentage of unassigned timepoints'});
...
% The text is changed
set(handles.TVIS,'String',['Zscore: ',sprintf('%.1f',get(hObject,'Value'))]);

% The colorbar graph is modified to suit the new threshold value
cla(handles.ColorbarCAP);
handles.ColorbarCAP = Create_CAP_colorbar(-1,1,0.5,get(hObject,'Value'),'',handles.ColorbarCAP,'Horizontal','div','RdBu',1000);
...
for n_ds = 2:handles.n_datasets
    ...
    try
        [handles.TPM{n_ds},handles.Counts{n_ds},...
            ...
            Compute_Metrics_simpler_Tomas(handles.idx{n_ds},handles.FrameIndices{n_ds}.kept.active,...
...
tmp_cb = cbrewer('div','RdYlGn',1000);
tmp_cb(tmp_cb < 0) = 0;

% 2. Transition matrix for all subjects together


tmp_toplot = (mean(handles.TM{handles.ReferencePopulation},3));
...
% colormap(handles.TMGraph,flipud(tmp_cb));
    colormap(handles.TMGraph,'jet');

clear tmp_toplot

% Arbitrary setting of probability scale from 0 to 0.03
caxis(handles.TMGraph,[0 0.06]);
...
% colormap(handles.TM_Subject,flipud(tmp_cb));
    colormap(handles.TM_Subject,'jet');
...
handles.ColorbarTransMat = Create_CAP_colorbar(0,0.06,0.01,0,'',...
    handles.ColorbarTransMat,'Vertical','div','RdYlGn',1000);
...
for i = 2:handles.n_datasets
%     tmp_toplot = [tmp_toplot; handles.TPM{i}; 0*ones(5,handles.SubjSize.TP)];
     tmp_toplot = [tmp_toplot; handles.TPM{i}];
end
tmp_toplot = tmp_toplot(1:end-5,:);

custom_cm = cbrewer('div','RdYlGn',1000); 
...
%     colormap(handles.DynStates,(custom_cm));
      colormap(handles.DynStates,'jet');
...
try
    ...
    [~,~,handles.ViolinRawCounts] = MakeViolin(tmp_toplot,...
        handles.ViolinRawCounts,leg_viol,'Time Coverage (%)',handles.PopColor,handles.n_datasets,handles.K);
catch
...
try
    ...
    [~,~,handles.ViolinNumberEntries] = MakeViolin(tmp_toplot,...
        handles.ViolinNumberEntries,leg_viol,'Ocurrence',handles.PopColor,handles.n_datasets,handles.K);
catch
...
try
    ...
    [~,~,handles.ViolinResilience] = MakeViolin(tmp_toplot,...
        handles.ViolinResilience,leg_viol,'Duration [ms]',handles.PopColor,handles.n_datasets,handles.K);
catch
...
%             tmp = DiscardNaN(M{i}(:,3:3+n_states-1))';
            ...
            for j = 1:n_states
                M2(i+(j-1)*n_pop,1:size(tmp,2)) = tmp(j,:).*1000;  % x1000 to turn into ms
            end
